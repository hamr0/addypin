...
Implemented 2026-05-04: tier 1 head tags on `web/index.html`
(title, meta description with privacy stance, canonical, OpenGraph
set, `twitter:card` summary, theme-color); `noindex, nofollow` on
`web/pin.html`, `web/manage.html`, `web/edit.html` so individual
pin pages and the owner area stay out of search indexes; static
`web/robots.txt` (disallow `/api/`, `/auth/`, `/manage`, `/edit/`,
sitemap reference) and `web/sitemap.xml` (single apex entry); both
wired into `server/http.js`'s `STATIC_FILES` map. JSON-LD skipped
on principle. `og:image` deferred — design task. PRD §2 amended to
note that pin pages are noindexed and declarative head tags don't
conflict with the "no analytics" line.

The privacy-claim invariant on the landing page is the
`drop a pin · get a link` banner plus the
`no accounts · no tracking · open source` footnote — keep both in
sync with the meta description if the wording ever shifts.
