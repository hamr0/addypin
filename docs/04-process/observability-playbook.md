...
// prints e.g. {"users":128,"active_7d":54,"orders":true→count}
...
**Pull script** — the "fetch the sources" step; pulselog owns staging → tar →
size-floor → atomic publish → rotation → JSONL line → failure email:
```bash
#!/bin/bash
...
- **Alert/digest mail is plain `sendmail`, unsigned by pulselog itself.**
  ...
  spam-foldered/bounced alert isn't your only notice: the JSONL lines, the off-box
  `file-age` dead-man's-switch (§8), and — the direct fix — a **fallback sink**.
- **Fallback alert sink (pulselog ≥ 0.7.0) — the real fix for the circular gap.**
  Add `fallback` to `alert`/`digest`/`backup`: a `command` (no shell) that gets
  the alert body on **stdin** and the subject as **`PULSELOG_SUBJECT`**, over a
  path *independent* of the primary MTA, so a dead/misconfigured mail path can't
  silence the tool. `when: "always"` (default) also survives an **async bounce
  after a clean local handoff** (what a broken SPF/DKIM/PTR actually is —
  `sendmail` returns 0, Gmail bounces later); `when: "on-primary-failure"` only
  fires when the local handoff fails.
  ```jsonc
  "alert": {
    "email": "you@example.com",
    "fallback": {
      "when": "always",
      // independent path — e.g. authenticated msmtp submission (Gmail-signed),
      // NOT the same localhost/Postfix account that may be the broken thing:
      "command": "/opt/<APP>/ops/fallback-notify.sh",
      "timeoutMs": 15000
      // or a push channel:  "command": "curl",
      //   "args": ["-m","10","-fsS","-d","@-","https://ntfy.sh/<secret-topic>"]
    }
  }
  ```
  Use `when: "always"` for **health alerts** (silent on green → fires only on
  real failures, no duplicate mail) and `when: "on-primary-failure"` for the
  **digest** (avoid a duplicate weekly email; delivery breakage is caught by the
  health fallback + the off-box mail watchdog). Best-effort: a broken sink is
  recorded in the JSONL, never fatal.
