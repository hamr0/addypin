...
> **Enforced in nginx per-vhost, deliberately not at the firewall.** The VPS also serves `ingest.late.fyi`, which is intentionally **not** behind Cloudflare (it resolves straight to `155.94.144.191`). A firewall rule restricting :443 to Cloudflare's ranges — the obvious way to do this — would have taken late.fyi's ingest endpoint offline. Anyone revisiting origin hardening on this box must check what else is on port 443 first. See [`ops/nginx/README.md`](../../ops/nginx/README.md).
