...
**Cloudflare and the real client IP.** Because CF fronts production, it terminates TLS at its edge and re-originates to the VPS — so nginx's raw TCP peer is a **Cloudflare edge IP, not the visitor**. Left uncorrected, that silently collapses every per-IP bucket above (addypin's *and* knowless's) onto a handful of CF edge addresses, **shared by every visitor routed through the same CF colo**. It is not a bypass — the keys stay un-forgeable — but a *collision*, and it fails toward over-throttling: unrelated users behind one colo would share the 30-logins/hr and 3-new-handles/hr budgets, so a busy or hostile neighbour could exhaust them and lock out legitimate users. This was masked for as long as the knowless race meant the caps never bound under load (§ above); `1.3.4` makes them bind, which makes the key load-bearing.
