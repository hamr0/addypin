...
- **Growth visibility (operator-only):** `server/stats.js` opens `${DATA_DIR}/addypin.db` read-only and prints `pins=<confirmed> customers=<distinct owner_handle>` to stdout. Local: one-shot via `npm run stats` or polled every 10 min via `npm run stats:log`. VPS: `ops/systemd/addypin-stats.{service,timer}` — daily oneshot, `EnvironmentFile=/etc/addypin/env` (so `DATA_DIR=/var/lib/addypin` pins both read and write paths), `ReadWritePaths=/var/lib/addypin` mirrors the main service, output appended to `${DATA_DIR}/stats.log` via `/bin/sh -c` (RHEL 8 systemd 239 lacks `StandardOutput=append:`). Live since 2026-04-30; `Persistent=true` so the timer catches up after downtime. No new tables, no event recording, no per-user dimensions — just the two row counts. Fits within §2's "no usage analytics" line: nothing is *recorded*, it's `SELECT COUNT(*)` against existing rows once a day.
