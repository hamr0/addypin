...
- **No public directory, feed, or discovery.** Pins are not listed anywhere. A shortcode is the only way in. Owner data (`/api/me/pins`, `PATCH`/`DELETE /api/pins`) is **auth-gated** (401/403 to anonymous requests) — that gate is the privacy control, not `robots.txt`. App/UI pages (`/SHORTCODE`, the wildcard subdomain, `/manage`, `/edit/…`) ship `<meta name="robots" content="noindex, nofollow">` and the public coords lookup carries `X-Robots-Tag: noindex`, so they stay out of search while remaining crawlable (a `Disallow`-ed page is never fetched, so its `noindex` is never seen — the two cancel, so we deliberately do not `Disallow`). Only `addypin.com` is indexable and the only URL in `sitemap.xml`. Declarative head tags + `robots.txt`/`sitemap.xml`/`llms.txt` are expected (see `docs/04-process/privacy-seo.md`) — open-web machine-readability, not analytics. The marketing apex carries no PII, so per the playbook's operator-guardrail it is **allow-all for named AI crawlers** (GPTBot/ClaudeBot/OAI-SearchBot/PerplexityBot — on the record, not the implicit `*` default) and ships an `og.png` link-preview banner as project identity. None of this conflicts with the line above.
...
**knowless layers its own per-IP caps underneath these** (defaults, not currently overridden in production): 30 login requests/hr (`login_ip`) and 3 new handles/hr (`create_ip`). addypin passes a `sourceIp` on all three `startLogin` call sites (`server/http.js:137`, `server/http.js:192`, `server/inbound.js`), so they are live. Both are disabled in dev (`maxLoginRequestsPerIpPerHour: 0`) because 127.0.0.1 would otherwise starve local testing. Before `knowless@1.3.4` these caps did **not** bind under concurrency — the counter was read before the awaited SMTP submit and incremented only after, so simultaneous requests from one IP all saw a stale count. Measured against a 3/hr cap with 10 concurrent logins from one IP: `1.1.9` sent 10 mails, `1.3.4` sends 3.

The per-IP keys derive from the **`X-Real-IP`** header, which nginx sets to `$remote_addr` via `proxy_set_header` — that *replaces* any client-supplied value, so it cannot be forged by the client. We deliberately do **not** key off the leftmost `X-Forwarded-For` token: nginx forwards XFF with `$proxy_add_x_forwarded_for` (append), so the leftmost token is whatever the client sent and is attacker-controlled — keying on it would let a client rotate the value to mint fresh buckets and evade every per-IP limit. If `X-Real-IP` is ever absent, `clientIp()` falls back to the *rightmost* XFF token (nginx's appended real-peer hop), then the socket address.

**Cloudflare and the real client IP.** Because CF fronts production, it terminates TLS at its edge and re-originates to the VPS — so nginx's raw TCP peer is a **Cloudflare edge IP, not the visitor**. Left uncorrected, that silently collapses every per-IP bucket above (addypin's *and* knowless's) onto a handful of CF edge addresses, **shared by every visitor routed through the same CF colo**. Here it fails as a *collision*, not a bypass — because nginx **sets** `X-Real-IP` to `$remote_addr` (replacing any client value) rather than appending, so the key stays un-forgeable and merely points at the wrong party. It fails toward over-throttling: unrelated users behind one colo share the 30-logins/hr and 3-new-handles/hr budgets, so a busy or hostile neighbour could exhaust them and lock out legitimate users. *(Had we appended or omitted the directive instead, the CDN would have made it strictly worse: Cloudflare **appends** the connecting IP to any client-supplied `X-Forwarded-For` rather than replacing it, leaving a client-forged element as the leftmost — and therefore trusted — one, which is a full cap bypass. A CDN layers the collision on top of the spoofing exposure; it doesn't replace it.)*

Be precise about what `1.3.4` changes here: the race it fixes let **concurrent** requests in one bucket slip the cap, but sequential requests still incremented and still bound — so the miskeyed bucket was *already* biting, merely leakily under burst. The upgrade makes enforcement reliable, so a wrong key bites more consistently. It **sharpens** this exposure rather than creating it, and the remedy is to fix the edge (below), not to hold back the upgrade.

Corrected 2026-07-14 by [`ops/nginx/00-cloudflare-realip.conf`](../../ops/nginx/00-cloudflare-realip.conf) (installed at `/etc/nginx/conf.d/`): `set_real_ip_from` for each of [Cloudflare's 22 published ranges](https://www.cloudflare.com/ips/) (15 IPv4 + 7 IPv6) plus `real_ip_header CF-Connecting-IP;`. nginx now restores `$remote_addr` to the true client *before* setting `X-Real-IP`, so no application change was needed. Scoping `set_real_ip_from` to CF's ranges is what keeps the header trustworthy: a client hitting the origin directly, around CF, is not in those ranges, so its forged `CF-Connecting-IP` is ignored and `$remote_addr` stays its real socket address. **Verified end-to-end** — the same request logged `141.101.76.184` (a CF edge) before the change and the true client address after (`83.83.96.5` over IPv4, `2001:1c02:…` over IPv6). Re-check the ranges if origin logs ever start showing CF IPs again; CF changes them rarely. (The list first shipped two ranges short — `131.0.72.0/22` and `2c0f:f248::/32`, restored 2026-07-15 — and now feeds three consumers that must stay in sync: `real_ip`, the `geo` guard below, and the `:443` firewall allow-list in [`ops/firewall/`](../../ops/firewall/).)

**Origin lock — Cloudflare is no longer optional (2026-07-14).** Testing the above surfaced a second gap: the origin answered requests on its raw IP, so a `curl --resolve` to `155.94.144.191` served the site and **skipped Cloudflare entirely** — along with the WAF and DDoS protection the edge exists to provide. It was never a rate-limit hole (real IPs were still attributed correctly under the fix above); it simply made the edge mandatory for honest users and optional for an attacker. addypin's four nginx server blocks now `return 403` unless the request genuinely arrived from a CF edge, via `geo $realip_remote_addr $addypin_cf_peer`. The guard must key on `$realip_remote_addr` — the *original* TCP peer — because `$remote_addr` is by now the restored visitor and can no longer report how the request arrived; an absent substitution (peer not trusted ⇒ not from CF) hits the default and 403s, which is the fail-closed direction.

> **Layered: an L3/L4 firewall drop under the per-vhost nginx 403.** When the origin lock first shipped (2026-07-14) it was enforced *only* per-vhost in nginx, because the VPS also served `ingest.late.fyi` grey-cloud (direct to `155.94.144.191`) and a firewall rule restricting :443 to Cloudflare's ranges would have taken it offline. As of **2026-07-15** ingest is behind Cloudflare too, so `:443` is now firewall-locked to CF's ranges in firewalld: packets from non-CF sources are dropped before they reach nginx — which is what actually protects the origin from a raw-IP volumetric flood, since the nginx 403 only fires *after* the TCP+TLS handshake completes. The constraint has **inverted**: every vhost on this box must now stay behind Cloudflare — if `ingest.late.fyi` ever returns to grey-cloud, the firewall lock must be removed first or it will be cut off. Ruleset, rationale, and a re-apply tool live in [`ops/firewall/`](../../ops/firewall/); the nginx layer in [`ops/nginx/README.md`](../../ops/nginx/README.md).

Verified after the change: through-CF `200` (site and `/api/health`); direct-to-origin `403` on :443 apex, :443 shortcode subdomain, and :80; `ingest.late.fyi` still reachable and its config untouched (0 guard lines vs addypin's 4).
...
- **External mail-delivery watchdog (new, off-VPS).** The home-server pulselog watch runs `addypin-mail-check.sh` (a `command` check): it reads the VPS `/var/log/maillog` over the backup SSH key and alerts if the latest send to the operator address bounced. Must be off-box — the on-VPS health check runs as unprivileged `addypin` (can't read maillog) and its alert would ride the same broken mail path. **Invariant it guards:** `mail.addypin.com` stays **DNS-only** in Cloudflare (→ `155.94.144.191`); if it's proxied, SPF (`ip4:155.94.144.191`), DKIM (`addypin2026._domainkey`), and PTR all break and Gmail bounces every message — the failure mode that once silently killed the weekly digest for a month.
...
- **Growth visibility (operator-only):** Owned by [`pulselog`](https://github.com/hamr0/pulselog)'s digest mode, write-once-no-tables. `server/stats.js --metrics-json` opens `${DATA_DIR}/addypin.db` read-only and prints `{"pins":<confirmed>,"customers":<distinct owner_handle>}` — that's addypin's only customization; pulselog owns snapshot/history/render/email. `ops/systemd/addypin-stats-digest.{service,timer}` (Sun 09:00 UTC) runs `pulselog --digest --config ops/pulselog/digest.config.json`: the metricsCommand collects the two numbers, pulselog appends one snapshot line to `${DATA_DIR}/stats.jsonl`, groups history by ISO week, and mails the week-over-week table to `avoidaccess@gmail.com` via sendmail → Postfix → OpenDKIM (DKIM-signed, same path as the `SHORTCODE@` auto-reply). The digest run *is* the weekly snapshot, so the old daily `addypin-stats` timer is retired. Enrichment: pulselog's `flightlog` rollup adds a 7-day error count + top error names (counts only, never messages) from `errors.jsonl`. Fits §2's "no usage analytics": nothing is *recorded*, it's `SELECT COUNT(*)` against existing rows once a week.
