...
> ⚠️ **Known defect — behind Cloudflare, `$remote_addr` is not the client.** Since CF began fronting production it terminates TLS at its edge and re-originates to the VPS, so nginx's TCP peer is a **Cloudflare edge IP**, not the visitor. nginx has **no `set_real_ip_from` / `real_ip_header` directives** (verified against `/etc/nginx/conf.d/addypin.conf`, 2026-07-14), so it never restores the real client from `CF-Connecting-IP`. Consequence: every per-IP bucket above — addypin's *and* knowless's — is keyed on a CF edge IP and therefore **shared by every visitor routed through the same CF colo**. The keys remain un-forgeable, so this is not a bypass; it is a *collision* problem, and it fails toward over-throttling: unrelated users behind one colo share the 30 logins/hr and 3-new-handles/hr budgets, so a busy or hostile colo neighbour can exhaust them and lock out legitimate users. This was masked while the knowless race meant the caps never bound under load; `1.3.4` makes them bind, which makes the coarse key load-bearing. **Fix:** add `set_real_ip_from` for [Cloudflare's published ranges](https://www.cloudflare.com/ips/) plus `real_ip_header CF-Connecting-IP;` in nginx, so `$remote_addr` is restored to the true client before `X-Real-IP` is set. Restricting `set_real_ip_from` to CF's ranges is what keeps the header trustworthy — without it, a direct-to-origin client could forge `CF-Connecting-IP`. Not yet applied; needs an nginx change on the VPS.
