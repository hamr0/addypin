...
CREATE INDEX IF NOT EXISTS idx_owner_handle ON pins (owner_handle);
...
export function createDb({ path }) {
    ...
    const stmts = {
        insert: db.prepare(`
            INSERT INTO pins (
                shortcode, coords_ciphertext, coords_iv, owner_handle,
        ...
        confirmAllByOwner: db.prepare(`
            UPDATE pins SET
                ...
                owner_email_ciphertext = NULL, owner_email_iv = NULL,
                reminder_sent_at = NULL
            WHERE owner_handle = ? AND status = 'unconfirmed'
        `),
    ...
    function insertPin({
        shortcode, ciphertext, iv, ownerHandle, status, createdAt, expiresAt,
        emailCiphertext = null, emailIv = null,
    }) {
        if (!isAvailable(shortcode)) {
            throw new Error(`shortcode '${shortcode}' is unavailable`);
        }
        if (typeof ownerHandle !== 'string' || !/^[a-f0-9]{64}$/.test(ownerHandle)) {
            throw new Error('ownerHandle must be a 64-char lowercase hex string');
        }
    ...
    function listPinsByOwner(ownerHandle) {
        return stmts.listByOwner.all(ownerHandle).map(toPin);
    }
    ...
    function confirmPinsByOwner(ownerHandle, now) {
        return stmts.confirmAllByOwner.run(now, ownerHandle).changes;
    }
