...
- **Edge:** **Cloudflare** fronts production (DNS-proxied / "orange cloud"). It terminates TLS at its edge and re-originates to the VPS — confirmed by the `server: cloudflare` response header and its ability to inject a managed `robots.txt`. It can also serve AI-bot rules at the edge (its "Block AI training bots" + managed-robots.txt features *can* override the repo `robots.txt`; both are now **off**, so the committed repo `robots.txt` is authoritative — verify empirically with `curl https://addypin.com/robots.txt`, and if policy ever diverges again, check the CF dashboard toggles, not just the repo file). Accepted SaaS exception per the dev rules.
...
M1–M11 shipped. Live `https://addypin.com` and `https://SHORTCODE.addypin.com` both resolve, `knowless@1.1.9` backs the auth flow (upstream walk-away release, feature-frozen at 1.0.0), wildcard cert renewed (auto-renewal via `certbot-renew.timer`). Path-based URLs (`addypin.com/SHORTCODE`) still resolve so old links don't break. Observability migrated to flightlog + pulselog: the VPS runs `pulselog` for health (`addypin-health.timer`) and the weekly stats digest (`addypin-stats-digest.timer`), `flightlog` captures app errors to `errors.jsonl`, and the home server runs `pulselog --backup` + an external watch (uptime + backup-freshness) in place of the old bash scripts and Kuma. Discoverability shipped too (og.png, JSON-LD, llms.txt, sitemap `<lastmod>`, AI-crawler-aware `robots.txt`). **Cloudflare fronts production** (discovered during this deploy; accepted "for good"). The CF AI-bot controls are now **off** (Control AI crawlers → "Do not block" + Manage robots.txt → off), so the committed allow-all repo `robots.txt` is live and authoritative — verified: `curl https://addypin.com/robots.txt` returns only repo directives (no CF managed block, no `ai-train=no`, ClaudeBot/GPTBot `Allow: /`), and the edge returns `200` to a ClaudeBot/GPTBot user-agent. Pending tail: tighten the IAM policy from `AmazonRoute53FullAccess` to a scoped inline policy on hosted zone `Z1CHOY92OEU194`. **Deploy note:** the new VPS units call `/opt/addypin/node_modules/pulselog/bin/pulselog.js`, so they need `npm ci` to have run *post-deploy* and the unit files re-installed (`systemctl daemon-reload`) — see `ops/homeserver/README.md` for the home-server side. **Mail deliverability (2.0.18):** all VPS→Gmail mail had been silently bouncing (digest + every health alert) — not a pulselog/flightlog bug; the digest ran fine, the mail just never delivered. Cause was three Cloudflare-DNS faults, now fixed: `mail.addypin.com` is **DNS-only** (grey cloud → `155.94.144.191`; must never be CF-proxied), SPF is `v=spf1 ip4:155.94.144.191 -all`, and the DKIM key `addypin2026._domainkey` is published (OpenDKIM was signing all along). Verified with a live send (`250 2.0.0 OK`). A new off-VPS `addypin-mail-check.sh` pulselog `command` check on the home server now watches VPS→Gmail delivery so a recurrence pages instead of vanishing.
